...
public class DetailChangeService {
	...
	public boolean nomalchange(DetailForm detailForm) throws HibernateException {
		...
		if (detailForm.getName() != "" && detailForm.getName() != null) {
			detailBean.setName(detailForm.getName());
		}
		if (detailForm.getAge() != "" && detailForm.getAge() != null) {
			detailBean.setAge(detailForm.getAge());
		}
		if (detailForm.getBirthday() != "" && detailForm.getBirthday() != null) {
			detailBean.setBirthday(detailForm.getBirthday());
		}
		if (detailForm.getMajor() != "" && detailForm.getMajor() != null) {
			detailBean.setMajor(detailForm.getMajor());
		}
		if (detailForm.getGender() != "" && detailForm.getGender() != null) {
			detailBean.setGender(detailForm.getGender());
		}
		if (detailForm.getPassword() != null) {
			detailBean.setPassword(detailForm.getPassword());
			userbean.setPassword(MD5Util.getMD5String(detailForm.getPassword()));
			Emailer.sendMail(userbean.getEmail(), "UNSWbook password changed", "password changed");
		}
		if (detailForm.getEmail() != null) {
			Emailer.sendMail(userbean.getEmail(), "email changed", "email changed");
			detailBean.setEmail(detailForm.getEmail());
			userbean.setEmail(detailForm.getEmail());
